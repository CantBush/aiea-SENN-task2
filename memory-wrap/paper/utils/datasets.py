...
def get_SVHN(data_dir:str, batch_size_train:int, batch_size_test:int, batch_size_memory:int,
    ...
    if balanced:
        ...
        mem_loader = torch.utils.data.DataLoader(train_dataset, batch_size=batch_size_memory, sampler = sampler, drop_last=True, pin_memory = torch.cuda.is_available())     
    else:
        mem_loader = torch.utils.data.DataLoader(train_dataset, batch_size=batch_size_memory,pin_memory = torch.cuda.is_available(), shuffle=True, drop_last=True,worker_init_fn=seed_worker)
    train_loader = torch.utils.data.DataLoader( train_dataset, batch_size=batch_size_train,pin_memory = torch.cuda.is_available(), shuffle=True, drop_last=True,worker_init_fn=seed_worker)

    val_loader = torch.utils.data.DataLoader( val_dataset, batch_size=batch_size_test,pin_memory = torch.cuda.is_available(), shuffle=False,worker_init_fn=seed_worker)

    test_loader = torch.utils.data.DataLoader(test_data, batch_size=batch_size_test,pin_memory = torch.cuda.is_available(), shuffle=False,worker_init_fn=seed_worker)
...
def get_CIFAR10(data_dir:str, batch_size_train:int, batch_size_test:int,batch_size_memory:int,size_train:int=100000,seed:int=42)->List[torch.utils.data.DataLoader]:
    ...
    val_loader = torch.utils.data.DataLoader(val_dataset, batch_size=batch_size_test,pin_memory = torch.cuda.is_available(),worker_init_fn=seed_worker)

    test_loader = torch.utils.data.DataLoader(test_data, batch_size=batch_size_test,pin_memory = torch.cuda.is_available(),worker_init_fn=seed_worker)

    mem_loader = torch.utils.data.DataLoader(memory_data, batch_size=batch_size_memory,pin_memory = torch.cuda.is_available(), shuffle=True, drop_last=True,worker_init_fn=seed_worker)
